...
def putTextWithBackground(img, text, origin, font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=3, text_color=(255, 255, 255), bg_color=(0, 0, 0), thickness=3):
    """绘制带有背景的文本。
...
# 视频处理
def detect_and_track1(input_path: str, output_path: str, detect_class: int, model, tracker) -> Path:
    """
    处理视频，检测并跟踪目标。
...
# 在 detect_and_track 函数中添加如下代码：

def detect_and_track2(input_path: str, output_path: str, detect_class: int, model, tracker) -> Path:
    ...
    # 创建CSV文件，文件名为 test.csv
    csv_filename = Path(output_path) / "test.csv"
    csv_file = open(csv_filename, mode='w', newline='')
    csv_writer = csv.writer(csv_file)

    frame_number = 0  # 初始化帧数计数器
    while True:
        ...
        for x1, y1, x2, y2, Id in resultsTracker:
            width = x2 - x1
            height = y2 - y1
            csv_writer.writerow([frame_number, int(Id), x1, y1, width, height, -1, -1, -1, -1])
...
if __name__ == "__main__":
    ...
    detect_and_track1(input_path, output_path, detect_class, model, tracker)
